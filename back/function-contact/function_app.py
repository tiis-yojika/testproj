...
import logging
import hashlib
from utils import missing, get_db_connection, error_response, success_response, rows_to_dict_list
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                INNER JOIN
                    INQUIRY_DETAILS d ON i.inquiry_id=d.inquiry_id
                                        AND d.{target_field}=?
                                        AND d.inquiry_no=1
                LEFT JOIN
                    INQUIRY_DETAILS d2 ON i.inquiry_id=d2.inquiry_id
                GROUP BY
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                LEFT JOIN
                    EVENTS e ON i.event_id=e.event_id
                LEFT JOIN
                    USERS ur ON d.recipient=ur.id
                LEFT JOIN
                    USERS us ON d.sender=us.id
                WHERE
                    i.inquiry_id=?
                ''',
                (inquiry_id,)
        ...
        with get_db_connection() as conn:
            ...
            if not inquiry_id:
                cursor.execute(
                    '''
                    INSERT INTO
                        INQUIRY_INFO (event_id, subject, hashed_inquiry_id)
                    OUTPUT
                        INSERTED.inquiry_id
                    VALUES
                        (?, ?, '');
            ...
            cursor.execute(
                '''
                INSERT INTO
                    INQUIRY_DETAILS (inquiry_id, inquiry_no, main_text, recipient, sender)
                VALUES
                    (?, ?, ?, ?, ?)
                ''',
...
def receive_inquiries(req: func.HttpRequest) -> func.HttpResponse:
    ...
    recipient = body.get('recipient')
    if not recipient:
        return error_response("受信者idは必須です。", status=400)
